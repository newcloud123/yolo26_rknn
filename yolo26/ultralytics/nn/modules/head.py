...
class Detect(nn.Module):
    ...
    def forward_head(
        ...
        # 2. 去掉torch.cat，直接收集每个尺度的cls预测结果（不拼接）
        cls_outputs = [cls_head[i](x[i]) for i in range(self.nl)]
        
        # 3. 返回结果：6个核心输出（box_outputs[0/1/2] + cls_outputs[0/1/2]）
        return box_outputs[0],cls_outputs[0], box_outputs[1], cls_outputs[1], box_outputs[2], cls_outputs[2]
    ...
    def forward(
        ...
        if self.end2end:
            ...
            # ["boxes"[1,4,8400],"scores"[1,2,8400],"feats"[[1,64,80,80],[1,128,40,40],[1,256,20,20]]]
        if self.training:
            return preds
    ...
    def _get_decode_boxes(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        shape = x["feats"][0].shape  # BCHW [1,64,80,80]
